...
function Profile() {
  ...
  const handleLogClick = (log: AuditLog) => {
    ...
    // Ha törölt a termék ÉS nem admin, akkor megállítjuk
    if (!log.stock && !user?.admin) {
      alert("Ehhez a művelethez nincs jogosultságod, fordulj az adminhoz segítségért!");
      return; 
    }
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-6 space-y-10 animate-fade-in">
      {/* Felhasználói kártya - JAVÍTOTT STÍLUS */}
        ...
        <div className="px-10 pb-10">
          <div className="relative -top-10 flex flex-wrap items-end gap-6">
            ...
            <div className="pb-4 min-w-[200px]">
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">{user.nev}</h1>
              <p className="text-slate-500 text-base font-semibold flex items-center gap-2">
                <span className="opacity-60 font-mono text-sm">@{user.felhasznalonev}</span>
                <span className="text-slate-300">•</span>
                <span className={user.admin ? "text-indigo-600" : "text-emerald-500"}>
          ...
          <div className="border-t border-slate-100 pt-6 mt-2">
            ...
            >
              <span className="bg-red-50 p-2 rounded-xl group-hover:bg-red-100 transition-colors">🚪</span>
      ...
      {/* Audit Log Szekció - JAVÍTOTT TÁBLÁZAT */}
      <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-10">
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="bg-indigo-50 text-indigo-600 px-5 py-2 rounded-2xl font-black text-sm border border-indigo-100">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-separate border-spacing-y-2">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan={user.admin ? 4 : 3} className="py-20 text-center text-slate-400 font-bold italic">Betöltés...</td></tr>
              ) : logs.length > 0 ? (
                logs.map((log) => {
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
                          ...
                          {log.muvelet === "CREATE" ? "Létrehozás" : 
                           log.muvelet === "DELETE" ? "Törlés" : 
                           log.muvelet === "RESTORE" ? "Visszaállítás" : "Módosítás"}
                        </span>
                      </td>
                      <td className="p-4 text-sm font-bold text-slate-700">
                        {log.stock?.nev || <span className="text-slate-300 italic font-medium">Törölt termék</span>}
                        {isLinkable && (
                          <span className="opacity-0 group-hover:opacity-100 text-indigo-400 ml-3 transition-all inline-block translate-x-[-10px] group-hover:translate-x-0">↗</span>
                        )}
                      </td>
                      {user.admin && (
                        <td className="p-4 text-sm font-medium text-slate-500">
                          <span className="bg-slate-100 px-3 py-1 rounded-lg">{log.user.nev}</span>
                        </td>
                      )}
                    </tr>
                  );
                ...
                <tr><td colSpan={user.admin ? 4 : 3} className="py-20 text-center text-slate-400 italic">Nincs még rögzített tevékenység.</td></tr>
              )}
            </tbody>
          </table>
        </div>
