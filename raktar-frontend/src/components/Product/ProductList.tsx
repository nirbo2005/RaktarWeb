...
function ProductList() {
  ...
  const exportToExcel = async () => {
    ...
    worksheet.columns = [
      { header: "Terméknév", key: "nev", width: 25 },
      { header: "Gyártó", key: "gyarto", width: 20 },
      { header: "Kategória", key: "kategoria", width: 20 },
      { header: "Össz. Mennyiség (db)", key: "mennyiseg", width: 20 },
      { header: "Legkorábbi Lejárat", key: "lejarat", width: 20 },
    ];
    ...
    filteredAndSortedProducts.forEach((p) => {
      ...
      const row = worksheet.addRow({
        ...
        mennyiseg: totalQty,
        lejarat: earliestExpiry ? earliestExpiry.toLocaleDateString("hu-HU") : "Nem romlandó",
      });
    ...
    toast.fire({ icon: 'success', title: 'Excel riport kész! 📊' });
  };
  ...
  const handleBulkDelete = async () => {
    ...
    const result = await MySwal.fire({
      ...
      text: `${selectedIds.length} tétel törlése?`,
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        ...
        toast.fire({ icon: 'success', title: 'Tételek törölve.' });
      } catch (err) {
        MySwal.fire('Hiba', 'Sikertelen tömeges törlés.', 'error');
      }
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        MySwal.fire('Hiba', 'Sikertelen törlés.', 'error');
      }
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {filteredAndSortedProducts.length > 0 ? (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-200 dark:border-slate-800">
                    ...
                    <th className="p-6 cursor-pointer hover:text-blue-500" onClick={() => handleSort("nev")}>Termék {sortColumn === "nev" && (isAscending ? "↑" : "↓")}</th>
                    <th className="p-6 cursor-pointer hover:text-blue-500" onClick={() => handleSort("kategoria")}>Kategória {sortColumn === "kategoria" && (isAscending ? "↑" : "↓")}</th>
                    <th className="p-6 cursor-pointer hover:text-blue-500" onClick={() => handleSort("mennyiseg")}>Össz. Készlet {sortColumn === "mennyiseg" && (isAscending ? "↑" : "↓")}</th>
                    ...
                    return (
                      <React.Fragment key={p.id}>
                        <tr className={`transition-colors group ${selectedIds.includes(p.id) ? "bg-blue-50/50 dark:bg-blue-900/20" : "hover:bg-blue-50/30 dark:hover:bg-blue-900/10"}`}>
                          ...
                          <td className="p-6"><span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-black uppercase text-slate-600 dark:text-slate-400">{p.kategoria}</span></td>
                          <td className="p-6"><span className={`px-4 py-1.5 rounded-full text-xs font-black border ${totalQty < p.minimumKeszlet ? 'bg-red-50 text-red-600 border-red-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}>{totalQty} db</span></td>
                          {canEdit && (
                        ...
                        {isExpanded && (
                          <tr className="bg-slate-50/50 dark:bg-slate-900/50">
                            <td colSpan={10} className="p-6 border-l-4 border-blue-500">
                              <div className="pl-6">
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Sarzsok</h4>
                                  ...
                                  {p.batches?.map(batch => (
                                    <div key={batch.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center group">
                                      <div>
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getDateStyles(batch.lejarat)}`}>{batch.lejarat ? new Date(batch.lejarat).toLocaleDateString('hu-HU') : 'Nem romlandó'}</span>
                                      </div>
                                      <div className="text-2xl font-black text-blue-600 dark:text-blue-400">{batch.mennyiseg} <span className="text-xs">db</span></div>
                                    </div>
                                  ))}
                                </div>
                              </div>
                  ...
                  <div key={p.id} className={`rounded-[2rem] p-6 border shadow-lg relative transition-all ${selectedIds.includes(p.id) ? "bg-blue-50/50 dark:bg-blue-900/40 border-blue-400" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800"}`}>
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex flex-col gap-2 mr-8">
                        <span className="px-3 py-1 rounded-lg text-[10px] font-black bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">{p.kategoria}</span>
                        <span className={`px-3 py-1 rounded-lg text-[10px] font-black border ${totalQty < p.minimumKeszlet ? 'bg-red-50 text-red-600 border-red-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}>📦 {totalQty} DB</span>
                      </div>
                    </div>
                    <div className="mb-6">
                    ...
                    <button onClick={() => toggleRow(p.id)} className="w-full mb-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl text-xs font-black uppercase tracking-widest text-slate-500 flex justify-between items-center px-4">
                      <span>Sarzsok ({p.batches?.length || 0})</span>
                        ...
                        {p.batches?.map(batch => (
                          <div key={batch.id} className="flex justify-between items-center bg-slate-100 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                            <div>
                              ...
                              <span className={`text-[9px] font-bold uppercase ${getDateStyles(batch.lejarat)} px-1 rounded`}>{batch.lejarat ? new Date(batch.lejarat).toLocaleDateString() : 'N/A'}</span>
                            </div>
                            <span className="font-black text-blue-600">{batch.mennyiseg} db</span>
                          </div>
                        ))}
                      </div>
                    )}
                    <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                      <button onClick={() => navigate(`/product/${p.id}`)} className="flex-1 bg-slate-100 dark:bg-slate-800 py-3 rounded-xl font-bold text-xs uppercase">Részletek</button>
                      {canEdit && (
                        <>
          ...
          <div className="py-32 text-center bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 transition-all">
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tighter italic">Minden polc rendben</h2>
            <p className="text-slate-400 font-bold uppercase text-xs tracking-[0.3em] mt-2">Nincs beavatkozást igénylő tétel</p>
          </div>
        )}
      </div>
    </div>
