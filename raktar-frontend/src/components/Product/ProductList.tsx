...
const MySwal = Swal.mixin({
  customClass: {
    ...
    cancelButton:
      "bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-8 py-3 rounded-2xl font-black uppercase text-xs tracking-widest transition-all active:scale-95 mx-2",
  },
...
type SortColumn = "nev" | "kategoria" | "mennyiseg" | "lejarat";

const getTotalQuantity = (product: Product) => {
  return product.batches?.reduce((sum, b) => sum + b.mennyiseg, 0) ?? 0;
};
...
const getEarliestExpiry = (product: Product) => {
  ...
  const dates = product.batches
    .filter((b) => b.lejarat)
    .map((b) => new Date(b.lejarat!).getTime());
...
function ProductList() {
  ...
  useAutoRefresh(() => fetchProducts(false));
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let list = products.filter((p) => {
      ...
      const matchesSearch =
        ...
        p.gyarto.toLowerCase().includes(searchLower) ||
        p.kategoria.toLowerCase().includes(searchLower);
        
      const matchesAlert = !showAlertsOnly || getAlertPriority(p) > 0;
    ...
    list.sort((a, b) => {
      ...
      if (sortColumn === "mennyiseg") {
        comp = getTotalQuantity(a) - getTotalQuantity(b);
      } else if (sortColumn === "lejarat") {
        const aExp = getEarliestExpiry(a)?.getTime() || Number.MAX_SAFE_INTEGER;
        const bExp = getEarliestExpiry(b)?.getTime() || Number.MAX_SAFE_INTEGER;
        comp = aExp - bExp;
      } else if (sortColumn === "kategoria") {
        comp = a.kategoria.localeCompare(b.kategoria);
      } else {
        comp = a.nev.localeCompare(b.nev);
      }
      return isAscending ? comp : -comp;
    });
    return list;
  }, [products, sortColumn, isAscending, showAlertsOnly, searchTerm]);
  ...
  const getProductStyles = (p: Product, isSelected: boolean) => {
    const totalQty = getTotalQuantity(p);
    const earliestExpiry = getEarliestExpiry(p);
    const now = new Date();
    now.setHours(0, 0, 0, 0);
    ...
    if (isCritical) {
      return base + "bg-rose-50/50 dark:bg-rose-500/5 border-rose-500 hover:bg-rose-100/50 dark:hover:bg-rose-500/10";
    }
    if (isWarning) {
      return base + "bg-amber-50/50 dark:bg-amber-500/5 border-amber-500 hover:bg-amber-100/50 dark:hover:bg-amber-500/10";
    }

    return base + "bg-white dark:bg-slate-900 border-transparent hover:bg-slate-50 dark:hover:bg-slate-800/40";
  };

  const renderExpiryInfo = (p: Product) => {
    ...
    if (diffDays <= 0) {
      badgeClass = "bg-rose-100 text-rose-700 border-rose-300 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800";
    } else if (diffDays <= 7) {
      badgeClass = "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800";
    }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="relative flex justify-end w-full md:w-auto z-20">
              {canEdit && (
                <div className="flex items-center">
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`shrink-0 whitespace-nowrap px-4 py-3 rounded-2xl font-black uppercase text-xs shadow-lg transition-transform active:scale-95 border ${
                        isSelectionMode
                  ...
                  <button
                    ...
                    className={`shrink-0 p-3.5 rounded-2xl shadow-lg transition-all z-10 relative border ${
                      isTopMenuOpen
                        ? "bg-slate-200 dark:bg-slate-800 border-transparent text-slate-900 dark:text-white"
            ...
            <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-6">
              {paginatedProducts.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`rounded-[2rem] p-6 border shadow-sm relative transition-all ${
                      selectedIds.includes(p.id) ? "bg-blue-50/50 dark:bg-blue-900/20 border-blue-500" : "bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800"
                    }`}
                  >
                    {canEdit && isSelectionMode && (
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700" onClick={() => handleShowQR(p)}>
                        <QRCodeSVG value={`${window.location.origin}/product/${p.id}`} size={50} />
                      </div>
                      <div className="flex flex-col gap-2 mr-8 items-end">
                        <span className="px-3 py-1 rounded-lg text-[10px] font-black bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 inline-block whitespace-nowrap">
                          {t(`product.categories.${p.kategoria}`)}
                        </span>
                        <span className={`px-3 py-1 rounded-lg text-[10px] font-black border inline-block whitespace-nowrap text-center ${
                            totalQty === 0
                              ? "bg-rose-100 text-rose-700 border-rose-300 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800"
                              : totalQty <= p.minimumKeszlet
                              ? "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800"
                    ...
                    <div className="mb-6">
                      <h3 className="text-2xl font-black tracking-tight cursor-pointer hover:text-blue-500 dark:text-white leading-tight" onClick={() => navigate(`/product/${p.id}`)}>
                        {p.nev}
                      </h3>
                      <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 whitespace-nowrap">
                        {p.gyarto}
                      </p>
                    </div>
                    ...
                    <button onClick={() => toggleRow(p.id)} className={`w-full mb-4 py-3 px-5 rounded-xl text-xs font-black uppercase tracking-widest flex justify-between items-center transition-all border whitespace-nowrap ${
                        isExpanded
                          ? "bg-blue-500 text-white border-blue-500 shadow-md"
                    ...
                    {isExpanded && (
                      <div className="mb-6 space-y-2">
                        {p.batches && p.batches.length > 0 ? (
                          p.batches.map((batch) => (
                            <div key={batch.id} className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                              <div>
                    ...
                    <div className="flex gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      {canEdit && (
                        <>
                          <button onClick={() => navigate(`/modify/${p.id}`)} className="bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 p-3 rounded-xl flex items-center justify-center transition-colors">✏️</button>
                          <button onClick={() => handleDelete(p.id)} className="bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400 p-3 rounded-xl flex items-center justify-center transition-colors">🗑️</button>
                        </>
                      )}
                    </div>
                  </div>
            ...
            <div className="mt-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] p-6 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-xl">
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  className="bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 py-3 px-4 outline-none cursor-pointer focus:ring-2 focus:ring-blue-500 shadow-sm"
                >
              ...
              <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-2xl p-1.5 shadow-inner">
                <button
