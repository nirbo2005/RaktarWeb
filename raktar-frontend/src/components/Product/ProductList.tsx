...
function ProductList() {
  ...
  const exportToExcel = async () => {
    ...
    worksheet.columns = [
      ...
      {
        header: t("product.list.excel.manufacturer"),
        key: "gyarto",
        width: 20,
      },
      ...
      {
        header: t("product.list.excel.earliestExpiry"),
        key: "lejarat",
        width: 20,
      },
    ];
    ...
    filteredAndSortedProducts.forEach((p) => {
      ...
      const row = worksheet.addRow({
        ...
        mennyiseg: totalQty,
        lejarat: earliestExpiry
          ? earliestExpiry.toLocaleDateString("hu-HU")
    ...
    saveAs(
      new Blob([buffer]),
      `raktar_export_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  ...
  const handleBulkDelete = async () => {
    ...
    const result = await MySwal.fire({
      ...
      text: t("product.list.alerts.bulkDeleteText", {
    ...
    if (result.isConfirmed) {
      try {
        ...
        toast.fire({
          icon: "success",
          title: t("product.list.alerts.bulkDeleted"),
        });
      } catch (err) {
        MySwal.fire(
          t("common.error"),
          t("product.list.alerts.bulkDeleteFailed"),
          "error",
        );
      }
    }
  ...
  const handleDelete = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        toast.fire({
          icon: "success",
          title: t("product.list.alerts.deleted"),
        });
      } catch (err) {
        MySwal.fire(
          t("common.error"),
          t("product.list.alerts.deleteFailed"),
          "error",
        );
      }
    }
          ...
          <div className="flex flex-wrap gap-3 w-full xl:w-auto xl:justify-end">
            {canEdit && isSelectionMode && selectedIds.length > 0 && (
              <button
                ...
                className="flex-1 sm:flex-none bg-red-600 text-white px-6 py-4 rounded-2xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all"
              >
            ...
            {canEdit && (
              <>
                <button
                  ...
                  className={`flex-1 sm:flex-none px-6 py-4 rounded-2xl font-bold border-2 text-xs uppercase tracking-widest transition-all ${showAlertsOnly ? "bg-red-600 border-red-400 text-white" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400"}`}
                >
                  {showAlertsOnly
                    ? t("product.list.filterErrors")
                    : t("product.list.filterAll")}
                </button>
                <button
                  onClick={exportToExcel}
                  className="flex-1 sm:flex-none bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-4 rounded-2xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all"
                >
                  {t("product.list.export")}
                </button>
                <button
                  onClick={toggleSelectionMode}
                  className={`flex-1 sm:flex-none px-6 py-4 rounded-2xl font-black uppercase text-xs shadow-lg transition-all ${isSelectionMode ? "bg-slate-700 text-white" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600"}`}
                >
                  {isSelectionMode
                    ? t("product.list.selectModeOn")
                    : t("product.list.selectModeOff")}
                </button>
                <button
                  onClick={() => navigate("/add")}
                  className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all"
                >
                ...
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-200 dark:border-slate-800">
                    ...
                    <th className="p-6 text-center w-24">
                      {t("product.list.headers.qr")}
                    </th>
                    <th
                      className="p-6 cursor-pointer hover:text-blue-500 text-left"
                      onClick={() => handleSort("nev")}
                    >
                      {t("product.list.headers.product")}{" "}
                      {sortColumn === "nev" && (isAscending ? "↑" : "↓")}
                    </th>
                    <th
                      className="p-6 cursor-pointer hover:text-blue-500 text-left"
                      onClick={() => handleSort("kategoria")}
                    >
                      {t("product.list.headers.category")}{" "}
                      {sortColumn === "kategoria" && (isAscending ? "↑" : "↓")}
                    </th>
                    <th
                      className="p-6 cursor-pointer hover:text-blue-500 text-left"
                      onClick={() => handleSort("mennyiseg")}
                    >
                      {t("product.list.headers.totalStock")}{" "}
                      {sortColumn === "mennyiseg" && (isAscending ? "↑" : "↓")}
                    </th>
                    {canEdit && (
                      <th className="p-6 text-right">
                        {t("product.list.headers.actions")}
                      </th>
                    )}
                  </tr>
                </thead>
                    ...
                    return (
                      <React.Fragment key={p.id}>
                        ...
                        >
                          ...
                          >
                            <div
                              className={`transform transition-transform ${isExpanded ? "rotate-90 text-blue-500" : "text-slate-400"}`}
                            >
                              ▶
                            </div>
                          </td>
                          ...
                          <td className="p-6 text-center">
                            <div
                              className="inline-block bg-white p-1.5 rounded-xl border border-slate-100 cursor-zoom-in shadow-sm hover:scale-110 transition-transform"
                              onClick={() => handleShowQR(p)}
                            >
                              <QRCodeSVG
                                value={`${window.location.origin}/product/${p.id}`}
                                size={35}
                              />
                            </div>
                          </td>
                          <td className="p-6 text-left">
                            ...
                            <div className="text-slate-400 text-xs uppercase font-bold tracking-tight">
                              {p.gyarto}
                            </div>
                          </td>
                          <td className="p-6 text-left">
                            ...
                            <td className="p-6 text-right">
                              <div className="flex justify-end gap-3">
                                <button
                                  ...
                                  className="bg-slate-100 dark:bg-slate-800 hover:bg-blue-600 hover:text-white p-2.5 rounded-xl text-slate-600 dark:text-slate-300 shadow-sm transition-all"
                                >
                                  ✏️
                                </button>
                                <button
                                  onClick={() => handleDelete(p.id)}
                                  className="bg-slate-100 dark:bg-slate-800 hover:bg-red-600 hover:text-white p-2.5 rounded-xl text-slate-600 dark:text-slate-300 shadow-sm transition-all"
                                >
                                  🗑️
                                </button>
                              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mb-6">
                      ...
                      <p className="text-blue-600 dark:text-blue-500 font-bold text-xs uppercase tracking-widest mt-2">
                        {p.gyarto}
                      </p>
                    </div>
                    <button
                      onClick={() => toggleRow(p.id)}
                      className="w-full mb-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl text-xs font-black uppercase tracking-widest text-slate-500 flex justify-between items-center px-4"
                    >
                      <span>
                    ...
                    {isExpanded && (
                      <div className="mb-6 space-y-2">
                        {p.batches?.map((batch) => (
                          ...
                          >
                            <div>
                              <span className="font-black italic text-sm block">
                                {batch.parcella}
                              </span>
                              <span
                                className={`text-[9px] font-bold uppercase ${getDateStyles(batch.lejarat)} px-1 rounded`}
                              >
                                {batch.lejarat
                                  ? new Date(batch.lejarat).toLocaleDateString()
                                  : t("product.list.nonPerishable")}
                              </span>
                            </div>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                      <button
                        onClick={() => navigate(`/product/${p.id}`)}
                        className="flex-1 bg-slate-100 dark:bg-slate-800 py-3 rounded-xl font-bold text-xs uppercase"
                      >
                      ...
                      {canEdit && (
                        <>
                          <button
                            onClick={() => navigate(`/modify/${p.id}`)}
                            className="bg-blue-600/10 text-blue-600 p-3 rounded-xl"
                          >
                            ✏️
                          </button>
                          <button
                            onClick={() => handleDelete(p.id)}
                            className="bg-red-600/10 text-red-600 p-3 rounded-xl"
                          >
                            🗑️
                          </button>
                        </>
