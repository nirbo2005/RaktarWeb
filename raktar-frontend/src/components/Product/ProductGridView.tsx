...
export const ProductGridView: React.FC = () => {
  ...
  const { user, triggerGlobalRefresh } = useAuth();
  ...
  const performMove = async (batchId: number, targetParcella: string) => {
    const allBatches = Object.values(batchesByPolc).flat();
    const draggedBatch = allBatches.find((b) => b.id === batchId);
    ...
    const currentWeightOnTarget = (batchesByPolc[targetParcella] || []).reduce(
      (s, b) => s + b.mennyiseg * b.suly,
      0,
    );
    const availableKg = MAX_SHELF_WEIGHT - currentWeightOnTarget;
    const canFit = Math.floor(availableKg / draggedBatch.suly);

    if (canFit <= 0) {
      MySwal.fire({
        title: t("product.grid.alerts.weightStop"),
        text: t("product.grid.alerts.shelfFull", { shelf: targetParcella }),
        icon: "error",
      });
      setDraggedBatchId(null);
      return;
    ...
    const absoluteMax = Math.min(draggedBatch.mennyiseg, canFit);
          ...
          ${
            draggedBatch.mennyiseg > canFit
              ? `<div class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-3 rounded-xl text-red-700 dark:text-red-400 text-[10px] font-black uppercase tracking-tighter">
                  ${t("product.grid.alerts.weightWarning", { max: canFit })}
                </div>`
              : ""
          }
    ...
    if (isConfirmed && moveQuantity) {
      ...
      try {
        await updateBatch(
          batchId,
          { parcella: targetParcella, mennyiseg: moveQuantity },
          user!.id,
        );
        ...
        <header className="mb-8 border-b border-slate-200 dark:border-slate-800 pb-6 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
              {t("product.grid.title")}
            </h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm italic tracking-wide">
              {t("product.grid.subtitle")}
            </p>
          </div>
                  ...
                  >
                    <div className="p-5 border-b dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 rounded-t-[2.5rem]">
                      ...
                      <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                        <div className={`h-full transition-all duration-500 ${weight > 1800 ? "bg-red-500" : "bg-blue-500"}`}
                          style={{ width: `${(weight / MAX_SHELF_WEIGHT) * 100}%` }} />
                      </div>
                    </div>
                    <ul className="p-4 space-y-3 flex-grow">
                      {shelfBatches.map((b) => {
                        ...
                        return (
                          <li key={b.id} draggable={canMove && !isMobile} ref={isTargetBatch ? highlightedProductRef : null}
                      ...
                      {shelfBatches.length === 0 && (
                        <li className="text-slate-300 dark:text-slate-700 text-[10px] font-black uppercase text-center py-10 tracking-widest italic opacity-50">
                          {t("product.grid.emptyShelf")}
                        </li>
                      )}
                    </ul>
                  </div>
