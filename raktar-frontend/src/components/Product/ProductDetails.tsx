...
function ProductDetails() {
  ...
  const handleDelete = async () => {
    ...
    const result = await MySwal.fire({
      ...
      text: t("product.details.alerts.deleteConfirmText", {
        name: product.nev,
      }),
      icon: "warning",
  ...
  const getLejaratStyle = (dateStr: string | Date | null) => {
    ...
    if (date <= now)
      return "bg-red-500/10 border-red-500/40 text-red-600 dark:text-red-400 border-2";
    if (date <= oneWeekLater)
      return "bg-amber-500/10 border-amber-500/40 text-amber-600 dark:text-amber-400 border-2";
    return "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200";
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 sm:p-8 transition-colors duration-300">
      <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 text-left transition-all">
        ...
        <div className="p-8 space-y-8">
          <div>
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 border-b border-slate-100 dark:border-slate-800 pb-2">
              {t("product.details.masterData")}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                <span className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase block mb-1 tracking-widest">
                  {t("product.details.weightUnit")}
                </span>
                <span className="text-2xl font-black text-slate-800 dark:text-slate-100 italic">
                  {product.suly} <span className="text-sm font-medium">kg</span>
                </span>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                <span className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase block mb-1 tracking-widest">
                  {t("product.details.minStock")}
                </span>
                <span className="text-2xl font-black text-slate-800 dark:text-slate-100 italic">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                <span className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase block mb-1 tracking-widest">
                  {t("product.details.purchasePrice")}
                </span>
                <span className="text-2xl font-black text-slate-800 dark:text-slate-100 italic">
                  {product.beszerzesiAr.toLocaleString()}{" "}
                  <span className="text-sm font-medium">Ft</span>
                </span>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                <span className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase block mb-1 tracking-widest">
                  {t("product.details.salePrice")}
                </span>
                <span className="text-2xl font-black text-slate-800 dark:text-slate-100 italic">
                  {product.eladasiAr.toLocaleString()}{" "}
                  <span className="text-sm font-medium">Ft</span>
                </span>
              </div>
            </div>
          ...
          <div>
            ...
            <div
              className={`p-8 rounded-3xl border transition-colors ${getStockStyle(totalQty, product.minimumKeszlet)} flex justify-between items-center`}
            >
              <div>
                <span className="opacity-60 text-[10px] font-black uppercase block mb-1 tracking-widest">
                  {t("product.details.totalStock")}
                </span>
          ...
          <div>
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 border-b border-slate-100 dark:border-slate-800 pb-2">
              {t("product.details.physicalLocation")}
            </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {product.batches.map((batch) => (
                  ...
                  >
                    ...
                    <div
                      className={`p-3 rounded-xl border ${getLejaratStyle(batch.lejarat)} transition-all duration-300`}
                    >
                      <span className="opacity-60 text-[9px] font-black uppercase block mb-1 tracking-widest">
                        {t("product.details.expiry")}
                      </span>
          ...
          {canEdit && !(product as any).isDeleted && (
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-50 dark:border-slate-800">
              <button
                ...
                className="flex-1 bg-slate-900 dark:bg-blue-600 text-white py-4 rounded-2xl font-black hover:bg-slate-800 dark:hover:bg-blue-500 shadow-lg uppercase tracking-widest text-[10px] italic transition-all"
              >
                {t("product.details.modify")}
              </button>
              <button
                onClick={handleDelete}
                className="flex-1 bg-red-600 text-white py-4 rounded-2xl font-black hover:bg-red-700 shadow-lg uppercase tracking-widest text-[10px] italic transition-all"
              >
