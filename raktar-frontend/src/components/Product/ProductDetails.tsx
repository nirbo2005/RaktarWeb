...
import socket from "../../services/socket"; // WebSocket importálva
...
function ProductDetails() {
  ...
  // Adatlekérés kiszervezve, hogy a Socket is hívhassa
  ...
  // JAVÍTÁS: WebSocket figyelő az élő frissítéshez (13, 14, 15 tesztek)
  useEffect(() => {
    const handleUpdate = (data: any) => {
      // Ha globális frissítés van, vagy konkrétan ezt a terméket érinti
      if (!data.productId || Number(data.productId) === Number(id)) {
        console.log("Élő frissítés érkezett a termékhez...");
        fetchProductData();
      }
    };
  ...
  // JAVÍTÁS: 16-os teszthez pontosított dátumkezelés
  const getLejaratStyle = (dateStr: string | Date | null) => {
    ...
    date.setHours(0, 0, 0, 0); // Csak a napot nézzük
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center transition-colors duration-300">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-blue-600 font-black tracking-widest uppercase text-xs text-center">Adatok lekérése...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 transition-colors duration-300">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-[3rem] p-10 shadow-2xl text-center border border-slate-200 dark:border-slate-800 transition-all">
          ...
          <p className="text-slate-400 dark:text-slate-500 mb-8 font-bold uppercase text-xs tracking-widest leading-relaxed text-center">Úgy tűnik, a keresett tétel nem található vagy törlésre került.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 sm:p-8 transition-colors duration-300">
      <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 transition-all">
        ...
        <div className="bg-blue-600 p-8 text-white relative overflow-hidden transition-colors flex justify-between items-start">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter uppercase italic leading-tight text-left">{product.nev}</h1>
        ...
        <div className="p-8 space-y-8">
          <div>
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 text-left">Mesteradatok</h3>
          ...
          <div>
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 text-left">Készlet Állapot</h3>
            <div className={`p-8 rounded-3xl border transition-colors ${getStockStyle(totalQty, product.minimumKeszlet)} text-left flex justify-between items-center`}>
              <div>
                ...
                {totalQty < product.minimumKeszlet && (
                  <span className="block text-[10px] font-black mt-2 uppercase tracking-tighter bg-white/20 dark:bg-black/20 px-2 py-0.5 rounded inline-block italic">🚨 Kritikus készlet (Minimum alatt!)</span>
                )}
              </div>
          ...
          <div>
            <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 text-left">Fizikai Elhelyezkedés (Sarzsok)</h3>
            {product.batches && product.batches.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {product.batches.map(batch => (
                  <div key={batch.id} className="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-3 group">
                    ...
                    <div className={`p-3 rounded-xl border ${getLejaratStyle(batch.lejarat)} text-left transition-all duration-300`}>
              ...
              <div className="text-center py-10 bg-slate-50 dark:bg-slate-800/30 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700">
                <span className="text-4xl mb-3 block opacity-50">🕸️</span>
                <p className="text-slate-500 font-bold uppercase text-xs tracking-widest">A termékből jelenleg egyetlen darab sincs raktáron.</p>
              </div>
            )}
          </div>
          ...
          {canEdit && !(product as any).isDeleted && (
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-50 dark:border-slate-800">
              <button onClick={() => navigate(`/modify/${product.id}`)} className="flex-1 bg-slate-900 dark:bg-blue-600 text-white py-4 rounded-2xl font-black hover:bg-slate-800 dark:hover:bg-blue-500 transition-all active:scale-95 shadow-lg uppercase tracking-widest text-[10px] italic">Cikktörzs / Készlet Módosítása</button>
              <button onClick={handleDelete} className="flex-1 bg-red-600 text-white py-4 rounded-2xl font-black hover:bg-red-700 transition-all active:scale-95 shadow-lg uppercase tracking-widest text-[10px] italic">Teljes Termék Törlése</button>
            </div>
          )}
        </div>
      </div>
