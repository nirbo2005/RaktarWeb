...
function Notifications() {
  ...
  const getIconAndColor = (type: string) => {
    switch (type) {
      case 'ERROR': return { icon: '🚨', bg: 'bg-red-50 dark:bg-red-900/20', border: 'border-red-200 dark:border-red-900/50', text: 'text-red-700 dark:text-red-400' };
      case 'WARNING': return { icon: '⚠️', bg: 'bg-amber-50 dark:bg-amber-900/20', border: 'border-amber-200 dark:border-amber-900/50', text: 'text-amber-700 dark:text-amber-400' };
      case 'SUCCESS': return { icon: '✅', bg: 'bg-emerald-50 dark:bg-emerald-900/20', border: 'border-emerald-200 dark:border-emerald-900/50', text: 'text-emerald-700 dark:text-emerald-400' };
      default: return { icon: 'ℹ️', bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-200 dark:border-blue-900/50', text: 'text-blue-700 dark:text-blue-400' };
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 text-left">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-500 hover:text-blue-600 transition-all shadow-sm">
              ←
            </button>
            <div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-slate-800 p-6 md:p-10 transition-colors">
                ...
                return (
                  <div 
                    ...
                    className={`relative p-6 rounded-3xl border-2 transition-all flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between text-left ${
                      notif.isRead 
                        ? 'bg-slate-50 dark:bg-slate-800/30 border-transparent opacity-60 grayscale-[50%]' 
