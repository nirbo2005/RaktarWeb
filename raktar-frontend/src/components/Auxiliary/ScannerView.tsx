//raktar-frontend/src/components/Auxiliary/ScannerView.tsx
...
function ScannerView() {
  ...
  const stockSummary = useMemo(() => {
    if (!product?.batches || product.batches.length === 0)
      return { total: 0, locations: t("auxiliary.scanner.noStock") };

    const total = product.batches.reduce(
      (sum, b) => sum + (b.mennyiseg || 0),
      0,
    );
    const uniqueLocations = Array.from(
      new Set(product.batches.map((b) => b.parcella)),
    ).filter(Boolean);
    ...
    return {
      ...
      locations:
        uniqueLocations.length > 0
          ...
          : t("auxiliary.scanner.unknownLocation"),
    };
  }, [product, t]);
    ...
    const startCamera = async () => {
      try {
        ...
        if (!err.toString().includes("is already scanning"))
          setCamError(t("auxiliary.scanner.camError"));
      }
    };
  ...
  const handleScanSuccess = async (decodedText: string) => {
    ...
    if (!isNaN(numericId)) {
      ...
      MySwal.fire({
        icon: "error",
        title: t("auxiliary.scanner.alerts.invalidCodeTitle"),
        text: t("auxiliary.scanner.alerts.invalidCodeText"),
      });
    }
  };
  ...
  const fetchProduct = async (id: number) => {
    ...
    try {
      ...
      MySwal.fire({
        icon: "error",
        title: t("auxiliary.scanner.alerts.noMatchTitle"),
        text: t("auxiliary.scanner.alerts.noMatchText"),
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 flex flex-col items-center justify-center transition-colors duration-300 text-left">
      ...
      <div
        id="reader"
        className={`w-full max-w-sm rounded-[3rem] overflow-hidden border-4 border-slate-200 dark:border-blue-500/30 bg-black relative shadow-2xl transition-all ${!product && !loading && !camError ? "block" : "hidden"}`}
      >
      ...
      {camError && !product && !loading && (
        <div className="w-full max-w-sm bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 p-6 rounded-[2.5rem] text-center">
          <p className="text-red-600 dark:text-red-400 font-black uppercase text-xs tracking-widest">
            {camError}
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="flex flex-col items-center gap-4 py-10">
          ...
          <div className="text-blue-600 dark:text-blue-400 font-black tracking-widest uppercase text-[10px] animate-pulse">
            {t("auxiliary.scanner.searching")}
          </div>
        </div>
      )}
      ...
      {product && (
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[3rem] p-10 shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in duration-300">
          <div className="flex justify-between items-start mb-2">
            ...
            <span className="text-slate-300 dark:text-slate-600 font-mono text-xs font-bold">
              #{product.id}
            </span>
          </div>
          ...
          <div className="space-y-6 mb-10">
            <div className="flex justify-between items-center">
              <span className="text-slate-400 font-black uppercase text-[10px] tracking-widest italic">
                {t("auxiliary.scanner.locations")}
              </span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-slate-400 font-black uppercase text-[10px] tracking-widest italic">
                {t("auxiliary.scanner.totalStock")}
              </span>
              <span
                className={`text-2xl font-black ${stockSummary.total <= product.minimumKeszlet ? "text-red-500" : "dark:text-white"}`}
              >
                {stockSummary.total} {t("common.pieces")}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-2xl font-black shadow-lg shadow-blue-500/20 active:scale-95 transition-all uppercase tracking-widest text-xs"
            >
              {t("auxiliary.scanner.detailsBtn")}
            </button>
            <button
              onClick={() => window.location.reload()}
              className="w-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 py-3 rounded-2xl font-black hover:bg-slate-200 dark:hover:bg-slate-700 transition-all uppercase text-[9px] tracking-[0.2em]"
            >
              {t("auxiliary.scanner.newScanBtn")}
            </button>
          </div>
        </div>
      )}
      ...
      {!loading && (
        <button
          onClick={() => navigate("/")}
          className="mt-12 text-slate-400 hover:text-blue-500 transition-colors font-black uppercase text-[10px] tracking-[0.3em] flex items-center gap-3"
        >
