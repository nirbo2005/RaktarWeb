    ...
    <nav className="bg-white dark:bg-black border-b border-slate-200 dark:border-slate-800 sticky top-0 z-[100] shadow-sm transition-colors text-left">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-2 md:gap-4">
          ...
          {user && !user.mustChangePassword && (
            <div className="hidden md:flex items-center gap-1 shrink-0">
              <Link to="/" className={linkStyle("/")}>
                {t("auxiliary.navbar.products")}
              </Link>
              <Link to="/grid" className={linkStyle("/grid")}>
                {t("auxiliary.navbar.overview")}
              </Link>
              <Link to="/scanner" className={linkStyle("/scanner")}>
                {t("auxiliary.navbar.scanner")}
              </Link>
            </div>
          )}
          ...
          {user && !user.mustChangePassword && (
            <div
              className="flex-1 max-w-md relative mx-2 hidden md:block"
              ref={searchRef}
            >
                  ...
                  <div className="absolute top-full right-0 mt-3 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-[120] overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2 text-left">
                    <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                      <span className="font-black text-sm uppercase tracking-widest text-slate-800 dark:text-slate-200">
                        {t("auxiliary.navbar.notifications")}
                      </span>
                      {unreadCount > 0 && (
                        <span className="text-xs font-bold text-blue-600 dark:text-blue-400">
                          {unreadCount} {t("auxiliary.navbar.new")}
                        </span>
                      )}
                    </div>
                    <div className="max-h-80 overflow-y-auto">
                      {recentNotifications.length > 0 ? (
                        ...
                        <div className="p-8 text-center text-slate-400 font-bold text-xs uppercase tracking-widest">
                          {t("auxiliary.navbar.noNotifications")}
                        </div>
                      )}
                    </div>
            ...
            {user ? (
              <div className="hidden md:flex items-center gap-3">
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 transition-all active:scale-95"
                >
                  🚪
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg transition-all active:scale-95"
                >
        ...
        <div className="px-4 py-6 space-y-3 text-left">
          {user ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link
                to="/login"
                className="bg-blue-600 text-white p-4 rounded-xl text-sm font-bold text-center"
              >
                {t("auxiliary.navbar.login")}
              </Link>
              <Link
                to="/register"
                className="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white p-4 rounded-xl text-sm font-bold text-center"
              >
