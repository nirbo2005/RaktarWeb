...
const Admin = ({ allUsers, pendingRequests, onRefresh }: Props) => {
  ...
  const validateField = (name: string, value: string) => {
    ...
    if (name === "nev") {
      ...
      else if (!/^[a-zA-ZÁÉÍÓÖŐÚÜŰáéíóöőúüű\s-]+$/.test(value)) error = t('admin.validation.nameNoNumbers');
    }
    ...
    <div className="p-8 space-y-10">
      {editingUser && (
        <div className="p-6 bg-blue-50 dark:bg-blue-900/10 border-2 border-blue-200 dark:border-blue-800 rounded-[2rem] animate-in fade-in slide-in-from-top-4 duration-300">
          <h4 className="font-black uppercase text-xs text-blue-600 dark:text-blue-400 mb-4 italic">{t('admin.editTitle', { name: editingUser.felhasznalonev })}</h4>
          <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 grid grid-cols-2 gap-2 mt-2">
                <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-xl font-black uppercase text-xs shadow-lg shadow-blue-600/20 transition-all active:scale-95">{t('common.save')}</button>
                <button type="button" onClick={() => setEditingUser(null)} className="bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 p-3 rounded-xl text-xs font-black uppercase hover:bg-slate-300 dark:hover:bg-slate-700 transition-all">{t('common.cancel')}</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="space-y-3">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-2">{t('admin.pendingRequests')}</h3>
        ...
        {pendingRequests.map((req) => (
          <div key={req.id} className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-2xl flex justify-between items-center transition-all">
            ...
            <div className="flex gap-2">
              <button onClick={async () => { await handleAdminRequest(req.id, "APPROVED"); toast.fire({ icon: 'success', title: t('admin.alerts.accepted') }); onRefresh(); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase active:scale-90 transition-all">OK</button>
              <button onClick={async () => { await handleAdminRequest(req.id, "REJECTED"); toast.fire({ icon: 'info', title: t('admin.alerts.rejected') }); onRefresh(); }} className="bg-rose-500 hover:bg-rose-600 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase active:scale-90 transition-all">X</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-2">{t('admin.systemUsers')}</h3>
