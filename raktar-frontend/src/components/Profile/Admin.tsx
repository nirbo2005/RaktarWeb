//raktar-frontend/src/components/Profile/Admin.tsx
import { useState } from "react";
import { updateProfile, handleAdminRequest, toggleUserBan, deleteUserPermanently } from "../../services/api";
...
const Admin = ({ allUsers, pendingRequests, onRefresh }: Props) => {
  ...
  const validateField = (name: string, value: string) => {
    ...
    if (name === "nev") {
      ...
      else if (!/^[a-zA-ZÁÉÍÓÖŐÚÜŰáéíóöőúüű\s-]+$/.test(value)) error = "Számok nem lehetnek a névben!";
    }
    if (name === "telefonszam" && (!value || value.length < 5)) {
      error = "A telefonszám kötelező!";
    }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (nError || tError) {
      toast.fire({ icon: 'error', title: 'Javítsd a hibákat!' });
      return;
    }
    ...
    try {
      const submitData = {
        ...adminEditForm,
        telefonszam: adminEditForm.telefonszam.startsWith('+') ? adminEditForm.telefonszam : `+${adminEditForm.telefonszam}`
      };

      await updateProfile(editingUser!.id, submitData);
      toast.fire({ icon: 'success', title: 'Felhasználó frissítve!' });
      setEditingUser(null);
      onRefresh();
    } catch (error: any) {
      MySwal.fire('Hiba!', error.message || 'Nem sikerült a mentés.', 'error');
    }
  };

  const handleToggleBan = async (u: User) => {
    if (u.id === currentUser?.id) {
      toast.fire({ icon: 'error', title: 'Saját magadat nem tilthatod ki!' });
      return;
    }
    try {
      ...
      toast.fire({
        icon: 'info',
        title: u.isBanned ? 'Fiók feloldva' : 'Fiók felfüggesztve'
      });
      onRefresh();
    } catch (error) {
      MySwal.fire('Hiba!', 'A művelet sikertelen.', 'error');
    }
  };

  const handleDeleteUser = async (u: User) => {
    if (u.id === currentUser?.id) {
      MySwal.fire('Hiba!', 'Saját admin fiókodat nem törölheted!', 'error');
      return;
    }

    const result = await MySwal.fire({
      title: 'Végleges törlés?',
      text: `Biztosan törlöd ${u.nev} felhasználót? Ez nem vonható vissza!`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        toast.fire({ icon: 'success', title: 'Felhasználó törölve' });
        onRefresh();
      } catch (error) {
        MySwal.fire('Hiba!', 'A törlés nem sikerült.', 'error');
      }
    }
  };
    ...
    <div className="p-8 space-y-10">
      {editingUser && (
        <div className="p-6 bg-blue-50 dark:bg-blue-900/10 border-2 border-blue-200 dark:border-blue-800 rounded-[2rem] animate-in fade-in slide-in-from-top-4 duration-300">
          <h4 className="font-black uppercase text-xs text-blue-600 dark:text-blue-400 mb-4 italic">Szerkesztés: {editingUser.felhasznalonev}</h4>
          <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={adminEditForm.rang} onChange={e => setAdminEditForm({ ...adminEditForm, rang: e.target.value })} className={inputClass("rang")}>
            ...
            <div className="md:col-span-1">
              <label className={labelClass}>Telefonszám (Kötelező)</label>
              <PhoneInput
                country={'hu'}
            ...
            <div>
              <label className={labelClass}>Új jelszó (Hagyd üresen, ha nem változik)</label>
              <input type="password" value={adminEditForm.ujJelszo} onChange={e => setAdminEditForm({ ...adminEditForm, ujJelszo: e.target.value })} className={inputClass("ujJelszo")} placeholder="••••••••" />
            </div>
            <div className="md:col-span-2 grid grid-cols-2 gap-2 mt-2">
                <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-xl font-black uppercase text-xs shadow-lg shadow-blue-600/20 transition-all active:scale-95">Mentés</button>
                <button type="button" onClick={() => setEditingUser(null)} className="bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 p-3 rounded-xl text-xs font-black uppercase hover:bg-slate-300 dark:hover:bg-slate-700 transition-all">Mégse</button>
            </div>
          </form>
        </div>
      )}
        ...
        {pendingRequests.map((req) => (
          <div key={req.id} className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-2xl flex justify-between items-center transition-all">
            <div>
              ...
              <p className="text-xs font-black dark:text-white">@{req.user.felhasznalonev} ➔ <span className="text-blue-600 dark:text-blue-400">{req.ujErtek}</span></p>
            </div>
            <div className="flex gap-2">
              <button onClick={async () => { await handleAdminRequest(req.id, "APPROVED"); toast.fire({ icon: 'success', title: 'Elfogadva' }); onRefresh(); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase active:scale-90 transition-all">OK</button>
              <button onClick={async () => { await handleAdminRequest(req.id, "REJECTED"); toast.fire({ icon: 'info', title: 'Elutasítva' }); onRefresh(); }} className="bg-rose-500 hover:bg-rose-600 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase active:scale-90 transition-all">X</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-2">Rendszer felhasználói</h3>
          ...
          {allUsers.map((u) => (
            <div key={u.id} className={`p-4 rounded-2xl border transition-all ${u.isBanned ? "bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/30" : "bg-slate-50 dark:bg-slate-800/40 border-slate-100 dark:border-slate-800 hover:border-blue-300 dark:hover:bg-slate-800/60"}`}>
              <div className="flex justify-between items-center">
                <div>
                  <p className={`font-black text-sm ${u.isBanned ? "text-red-600" : "dark:text-white"}`}>{u.nev} {u.rang === "ADMIN" ? "🛡️" : u.rang === "KEZELO" ? "📦" : "👁️"}</p>
                  <p className="text-[10px] text-slate-400 font-bold italic">@{u.felhasznalonev}</p>
                  {u.telefonszam && <p className="text-[9px] text-slate-500 font-mono mt-1">{u.telefonszam}</p>}
                </div>
