...
const ProfileAvatar: React.FC<ProfileAvatarProps> = ({ 
  ...
  const createIdolCanvas = async (imageSrc: string, pixelCrop: any): Promise<Blob> => {
    ...
    await new Promise((resolve, reject) => {
      img.onload = resolve;
      img.onerror = () => reject(new Error("Hiba a kép betöltésekor."));
    });
  ...
  const handleUpload = async () => {
    if (!image || !croppedAreaPixels || !onUploadSuccess || !user.id) {
        console.error("[DEBUG] Hiányzó adatok a feltöltéshez", { image: !!image, pixels: !!croppedAreaPixels, userId: user.id });
        return;
    }
    ...
    try {
      ...
      // Kényszerített típuskonverzió, ha a backend strictly numeric stringet vagy számot vár
      const updatedUser = await uploadAvatar(user.id.toString(), croppedImageBlob);
      
      onUploadSuccess(updatedUser);
      closeModal();
    } catch (error: any) {
      console.error("[DEBUG] Mentési hiba részletei:", error);
          ...
          <div className="p-10 space-y-8 text-center">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Fájl feltöltése</span>
              </button>
              ...
              <div className="flex flex-col items-center justify-center gap-4 p-8 rounded-3xl border-2 border-slate-100 dark:border-slate-800/50 bg-slate-50/30 dark:bg-slate-800/20">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Vagy nyomj CTRL+V-t</span>
              </div>
            </div>
            ...
            <div className="relative group">
              <input 
                type="text" 
                placeholder="Illessz be egy közvetlen kép URL-t..." 
          ...
          <div className="animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="flex gap-4 font-black uppercase italic tracking-tighter">
                ...
                >
                  {isUploading ? "Mentés folyamatban..." : "Kép Mentése"}
                </button>
              </div>
            </div>
          </div>
      ...
      >
        ...
        {!readonly && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur-[2px]">
            <span className="bg-white/20 px-4 py-2 rounded-full text-white text-[10px] font-black uppercase tracking-widest border border-white/30">Módosítás</span>
          </div>
        )}
      </div>
