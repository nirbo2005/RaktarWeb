      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-8 transition-colors">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 font-semibold mt-3 text-sm md:text-base transition-colors">
              Kifejezés:{" "}
              <span className="text-blue-500 underline decoration-2 underline-offset-4 font-bold">
                "{query}"
              </span>
            </p>
          </div>
          <div className="inline-block bg-white dark:bg-slate-900 px-6 py-2 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 font-bold text-xs md:text-sm tracking-widest uppercase self-start md:self-end transition-all">
        ...
        {loading ? (
          <div className="flex flex-col items-center py-20 gap-4 text-center">
            ...
            <p className="text-slate-400 dark:text-slate-500 font-black uppercase text-xs tracking-[0.2em]">
              Készlet átfésülése...
            </p>
          </div>
        ) : results.length > 0 ? (
          <div className="grid gap-6">
            {results.map((p) => (
              <div
                ...
                className="group bg-white dark:bg-slate-900 p-3 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl hover:shadow-2xl hover:shadow-blue-500/10 dark:hover:border-blue-500/30 transition-all duration-300 cursor-pointer flex flex-col md:flex-row items-center gap-4 md:gap-6"
              >
                ...
                <div className="flex-1 py-2 px-2 text-center md:text-left">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors ${getStatusBadge(p)}`}
                    >
                      {p.mennyiseg < 10 ? "Kritikus" : "Raktáron"}
                    </span>
                    <span className="text-slate-300 dark:text-slate-600 text-xs font-bold font-mono transition-colors">
                      #{p.id}
                    </span>
                  </div>
                ...
                <div className="w-full md:w-auto flex md:flex-col items-center justify-around md:justify-center gap-4 bg-slate-50 dark:bg-slate-800/50 md:bg-transparent p-4 md:p-8 rounded-[2rem] md:rounded-none transition-colors">
                  <div className="text-center">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase mb-1 transition-colors">
                      Hely
                    </p>
                  ...
                  <div className="text-center border-l md:border-l-0 md:border-t border-slate-200 dark:border-slate-800 pl-4 md:pl-0 md:pt-4 transition-colors">
                    ...
                    <span className="text-lg md:text-xl font-black text-slate-800 dark:text-white transition-colors">
                      {p.mennyiseg} db
                    </span>
                  </div>
                </div>
          ...
          <div className="text-center py-20 md:py-32 bg-white dark:bg-slate-900 rounded-[3rem] border-4 border-dashed border-slate-100 dark:border-slate-800 px-6 transition-all">
            <div className="text-6xl md:text-8xl mb-6 animate-bounce transition-all">
              🏜️
            </div>
            <h2 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white mb-2 italic tracking-tighter uppercase transition-colors">
              Nem találtunk semmit...
            </h2>
            <p className="text-slate-400 dark:text-slate-500 font-bold mb-8 max-w-sm mx-auto uppercase text-xs tracking-widest transition-colors">
              A tétel jelenleg nem szerepel a nyilvántartásban.
            </p>
            <button
              onClick={() => navigate("/")}
              className="w-full md:w-auto bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95 uppercase text-sm tracking-widest"
            >
              Vissza a kezdőlapra
            </button>
          </div>
