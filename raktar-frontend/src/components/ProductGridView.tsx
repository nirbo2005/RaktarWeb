...
const ProductGridView: React.FC = () => {
  ...
  // Tailwind osztályok a státuszokhoz
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 border-b pb-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500">Válassz parcellát a polcok megtekintéséhez</p>
          </div>
          {loading && <span className="animate-pulse text-blue-600 font-medium">Frissítés...</span>}
        </header>

        {/* Parcella választó háló */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Parcella térkép</h2>
          <div className="flex flex-col gap-4">
            {rows.map((row) => (
              <div key={row} className="flex gap-4 items-center">
                <span className="w-8 font-bold text-gray-400 text-xl">{row}</span>
        ...
        {/* Polcok és termékek megjelenítése */}
        <div className="mt-8">
          ...
          {selectedParcella ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-bold text-gray-800">{selectedParcella} Parcella polcai</h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {Array.from({ length: polcCount }, (_, i) => i + 1).map((polcIndex) => {
                  ...
                  return (
                    <div key={polcName} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                      <div className="bg-gray-800 p-3">
                        <h4 className="text-white font-semibold text-center">{polcIndex}. Polc</h4>
                      </div>
                      <div className="p-4 flex-grow">
                        <ul className="space-y-3">
                          {products && products.length > 0 ? (
                            products.map((p) => (
                              <li key={p.id} className={`p-3 rounded-lg border text-sm font-medium shadow-sm ring-1 ring-inset ${getStatusClass(p)}`}>
                                <div className="flex justify-between items-center">
                                  <span className="truncate pr-2">{p.nev}</span>
                                  <span className="font-bold whitespace-nowrap">{p.mennyiseg} db</span>
                                </div>
                              </li>
                            ))
                          ) : (
                            <li className="text-gray-400 text-center py-4 text-sm italic">Üres polc</li>
                          )}
                        </ul>
                      </div>
                    </div>
            ...
            <div className="text-center py-20 bg-gray-100 rounded-3xl border-2 border-dashed border-gray-300">
              <p className="text-gray-500 text-lg italic">Válassz egy parcellát a fenti térképen az adatok megjelenítéséhez!</p>
            </div>
          )}
        </div>
      </div>
